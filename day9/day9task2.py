"""
Day 9 task 2 Advent of Code 2023
...
def ifall0(line):
    last_num = [line[0]]   # list of last elements
    while not all(v == 0 for v in line):
        if len(line) == 1:
            return 0
        else:
            line = diff(line)
            last_num.append(line[0])
    result = 0      # substraction instead of sum
    for num in reversed(last_num[:-1]):
        result = num - result
    return result
